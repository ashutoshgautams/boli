...
const ROTATING_WORDS = [
  ...
  { word: 'Serendipity', meaning: 'a fortunate accident' },
  { word: 'Pragmatic', meaning: 'dealing with things realistically' },
...
export default function Hero() {
  ...
  return (
    <section className="relative min-h-[90vh] flex flex-col overflow-hidden bg-gradient-to-br from-warm-50 via-white to-warm-100">
      ...
      {/* Bottom: Vocabulary Challenge Teaser Strip */}
      <div className="relative z-10 bg-gradient-to-r from-primary-900 via-primary-800 to-primary-900 border-t border-primary-700/50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <Link href="/vocabulary-challenge" className="group flex items-center justify-between py-4 sm:py-5 gap-4">
            {/* Left: Label + rotating word */}
            <div className="flex items-center gap-4 min-w-0">
              <div className="flex-shrink-0 w-10 h-10 sm:w-11 sm:h-11 bg-yellow-400/15 rounded-xl flex items-center justify-center">
                <Brain className="w-5 h-5 sm:w-6 sm:h-6 text-yellow-400" />
              </div>
              <div className="min-w-0">
                <div className="flex items-center gap-2 mb-0.5">
                  <span className="text-xs sm:text-sm font-semibold text-yellow-400 uppercase tracking-wider">Daily Challenge</span>
                  <span className="hidden sm:inline-flex items-center gap-1 text-xs text-primary-300 bg-primary-700/50 px-2 py-0.5 rounded-full">
                    <Zap className="w-3 h-3" /> ELO Ranked
                  </span>
                </div>
                <div className="flex items-baseline gap-2 overflow-hidden">
                  <span
                    className={`text-white font-bold text-lg sm:text-xl transition-all duration-300 ${
                  ...
                  <span
                    className={`hidden sm:inline text-primary-300 text-sm transition-all duration-300 truncate ${
                      fade ? 'opacity-100' : 'opacity-0'
                    }`}
                  >
                    — {ROTATING_WORDS[wordIdx].meaning}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-4 sm:gap-6 flex-shrink-0">
              <div className="hidden md:flex items-center gap-5 text-primary-300 text-xs">
              ...
              <div className="flex items-center gap-2 px-4 sm:px-5 py-2 sm:py-2.5 bg-yellow-400 text-primary-900 rounded-xl font-bold text-xs sm:text-sm group-hover:bg-yellow-300 transition-all group-hover:scale-105">
                Play now
