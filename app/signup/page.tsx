...
import { Phone, Mail, Lock, ArrowRight, ArrowLeft, CheckCircle } from 'lucide-react';
...
export default function SignupPage() {
  ...
  const handleGoogleSignup = async () => {
    setIsLoading(true);
    try {
      // TODO: Implement Google OAuth with phone number
      await new Promise(resolve => setTimeout(resolve, 1000));
      window.location.href = '/dashboard';
    } catch (err) {
  ...
  return (
    <div className="auth-container">
            ...
            <form onSubmit={handleEmailSignup} className="space-y-5 mb-6">
              ...
              <div className="flex items-start gap-2">
                <input
                  ...
                  id="terms"
                  required
                  className="mt-1 w-4 h-4 rounded border-warm-300 text-primary-600 focus:ring-primary-500"
                />
      ...
      <Link
        href="/"
        className="mt-6 text-center block text-sm text-warm-600 hover:text-primary-600 transition-colors"
      >
        ← Back to home
      </Link>
    </div>
